...
export default function SignInScreen() {
  ...
  return (
    <View className="flex-1 justify-center bg-paper px-7">
      ...
      <Text className="font-display mb-1 text-[28px] text-ink">
        {mode === 'sign-in' ? 'Content de vous revoir' : 'Bienvenue sur Regain'}
      </Text>
      <Text className="font-body mb-7 text-sm text-ink-soft">
      ...
      <Controller
        ...
        render={({ field: { onChange, onBlur, value } }) => (
          <TextInput
            className="font-body mb-1 rounded-2xl border border-line bg-surface px-4 py-3.5 text-ink"
      ...
      />
      {errors.email ? <Text className="font-body mb-2 text-xs text-red-700">{errors.email.message}</Text> : null}
      ...
      <Controller
        ...
        render={({ field: { onChange, onBlur, value } }) => (
          <TextInput
            className="font-body mb-1 mt-2 rounded-2xl border border-line bg-surface px-4 py-3.5 text-ink"
      ...
      />
      {errors.password ? <Text className="font-body mb-2 text-xs text-red-700">{errors.password.message}</Text> : null}

      {serverError ? <Text className="font-body mt-2 text-xs text-red-700">{serverError}</Text> : null}
      {info ? <Text className="font-body mt-2 text-xs text-primary">{info}</Text> : null}
      ...
      <Pressable onPress={() => setMode(mode === 'sign-in' ? 'sign-up' : 'sign-in')} className="mt-4 items-center">
        <Text className="font-body text-sm text-ink-soft">
          ...
          <Text className="font-label text-primary">
